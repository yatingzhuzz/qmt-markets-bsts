...
# Extract the hold-out period + 2024 Q4
...
predict_all_windows_variables <- function(all_modelewall, hold_out_dataset) {
  ...
  for(window_num in 1:6) {
    ...
    for(var_name in names(all_modelewall)) {
      ...
      # Create period labels (80-87 plus Q4)
...
analyze_bsts_mcmc_trace_plots <- function(all_evaluations, all_models, best_models) {
  ...
  pdf("MCMC_trace_plot_EW_ALL_2015Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_EW_ALL_2015Q1.pdf", width = 12, height = 8)
...
format_window_range <- function(window) {
  train_ranges <- c(
    "1-77 (excluding hold-out 73-79)", 
    "1-82(excluding hold-out 73-79)",
    "1-87 (excluding hold-out 73-79)",
    "1-92 (excluding hold-out 73-79)",
    "1-97 (excluding hold-out 73-79)"
  )
...
# Save the workbook
saveWorkbook(wb, file = "Removed_pre_BWEW_2015.xlsx", overwrite = TRUE)
...
analyze_bsts_mcmc_trace_plots <- function(all_evaluations, all_models, best_models) {
  # Create a PDF device to save all plots
  pdf("MCMC_trace_plot_EW_BW_2015Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_EW_BW_2015Q1.pdf", width = 12, height = 8)
...
```{r}
# Complete Excel Results Creation with Full Backward Elimination Handling
# Modified create_enhanced_prediction_tables function - HANDLES BACKWARD ELIMINATION
create_enhanced_prediction_tables_revised <- function(all_evaluations, all_models, extended_test_sets, 
                                              crps_scores, lpd_scores, file_path, 
                                              aggregate_smape_results,
                                              ensemble_results) {
  ...
  for(dep_var in dep_vars) {
    ...
    # Handle backward elimination in prediction sections
    start_row <- 7
      ...
      tryCatch({
        ...
        if(!is.null(extended_test_sets[[dep_var]]) && 
          ...
          if(!is.null(model) && !is.null(model$coefficients)) {
            ...
            # FIND VARIABLES THAT SURVIVED BACKWARD ELIMINATION
            ...
            if(length(eliminated_vars) > 0) {
              cat("Variables eliminated by backward elimination for", dep_var, "window", window, ":", paste(eliminated_vars, collapse=", "), "\n")
              
              # ADD ELIMINATED VARIABLES AS ZEROS (they were deemed unimportant)
              for(eliminated_var in eliminated_vars) {
                last_row_predictors[[eliminated_var]] <- 0
              }
              
              # REORDER COLUMNS TO MATCH MODEL EXPECTATION
              last_row_predictors <- last_row_predictors[, model_vars, drop = FALSE]
            }
            
            if(length(selected_vars) > 0) {
              cat("Variables selected by backward elimination for", dep_var, "window", window, ":", paste(selected_vars, collapse=", "), "\n")
              
              # MAKE PREDICTION WITH COMPLETE VARIABLE SET (selected + eliminated as zeros)
              ...
              # NO VARIABLES SURVIVED BACKWARD ELIMINATION
              cat("No variables survived backward elimination for", dep_var, "window", window, "\n")
    ...
    for(window in top_5_median) {
      tryCatch({
        # SAME BACKWARD ELIMINATION HANDLING FOR MEDIAN SECTION
        if(!is.null(extended_test_sets[[dep_var]]) && 
           length(extended_test_sets[[dep_var]]) >= window &&
           !is.null(extended_test_sets[[dep_var]][[window]])) {
          ...
          if(!is.null(model) && !is.null(model$coefficients)) {
            ...
            # ADD ELIMINATED VARIABLES AS ZEROS (they were deemed unimportant by backward elimination)
            if(length(eliminated_vars) > 0) {
              for(eliminated_var in eliminated_vars) {
                last_row_predictors[[eliminated_var]] <- 0
              }
...
###############################################################################
## 7) create_excel_results_revised - MAIN FUNCTION WITH FULL FUNCTIONALITY
###############################################################################
create_excel_results_revised <- function(
  all_evaluations, 
  all_models, 
...
###############################################################################
## 8) USAGE EXAMPLE - Call this with your existing ensemble_results
###############################################################################
# Usage with YOUR EXACT variable names:
result <- create_excel_results_revised(
...
# Save the workbook
saveWorkbook(wb, file = "Removed_pre_StepEW_2015.xlsx", overwrite = TRUE)
...
analyze_bsts_mcmc_trace_plots <- function(all_evaluations, all_models, best_models) {
  # Create a PDF device to save all plots
  pdf("MCMC_trace_plot_EW_STEP_2015Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets_step, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_EW_Step_2015Q1.pdf", width = 12, height = 8)
...
create_excel_results_revised <- function(
  ...
  file_path <- "EW_Step_2015Q1.xlsx"  # CORRECT FILE NAME FOR STEPWISE
