...
```{r}
filepath <- "C:/Users/yatin/Desktop/Wells Fargo FS Q1-97 to Q4-24 Consolidation V3.0.xlsx"
mydata <- read_excel(
  path = filepath,
...
```{r}
# Extracting rows 73-79 from each dataset in dep_var_datasets
...
```{r}
# Define the hold-out period (rows 73-79)
hold_out_period <- 73:79#period

# Function to exclude the hold-out period from the dataset
exclude_hold_out_1 <- function(df) {
  # Exclude the rows that are in the hold-out period (73-79)
...
predict_all_rolling_windows_holdout <- function(all_models_rolling, hold_out_dataset) {
  ...
  for(window_num in 1:5) {  # 5 rolling windows
    ...
    for(var_name in names(all_models_rolling)) {
      ...
      if(num_rows <= 7) {
        row_ids <- 73:79#period
      } else {
        row_ids <- c(73:79, 112)#period
      }
...
analyze_bsts_mcmc_trace_plots_rolling <- function(all_evaluations, all_models, best_models) {
  ...
  pdf("MCMC_trace_plot_RW_ALL_2015Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets_rolling, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_RW_ALL_2015Q1.pdf", width = 12, height = 8)
...
run_sensitivity_analysis_parallel <- function() {
  ...
  # Initialize results
   sensitivity_results_rwall <- list()
  
  # For each dependent variable
  for (dep_var in names(best_models)) {
    cat("\n=== Starting analysis for", dep_var, "===\n")
     sensitivity_results_rwall[[dep_var]] <- list()
    ...
    for (i in 1:min(5, nrow(models_info))) {
      ...
      # Store results for this model
       sensitivity_results_rwall[[dep_var]][[model_name]] <- list(
        window = window_size,
        method = method,
  ...
  return( sensitivity_results_rwall)
}
...
cat("Starting sensitivity analysis at:", format(start_time), "\n")
 ...
 sensitivity_results_rwall <- run_sensitivity_analysis_parallel()
...
find_top_models <- function(all_models_rwall,  sensitivity_results_rwall) {
  top_models_summary <- list()
  
  # Process each dependent variable
  for(dep_var in names( sensitivity_results_rwall)) {
    cat("\n================================================================\n")
    ...
    if (dep_var %in% names( sensitivity_results_rwall)) {
      for (model_name in names( sensitivity_results_rwall[[dep_var]])) {
        model_data <-  sensitivity_results_rwall[[dep_var]][[model_name]]
...
# Example usage:
 top_model_results <- find_top_models(all_models_rwall,  sensitivity_results_rwall)
```
...
predict_and_create_ensemble <- function(all_models_rwall, top_model_results, hold_out_dataset) {
  ...
  for(dep_var in names(top_model_results)) {
    ...
    predict_model_worker <- function(params) {
      ...
      period_labels <- c(as.character(73:79), "Q4")#period
...
## ensemble result 
```{r}
combine_existing_results_rwall <- function(all_models_rwall, sensitivity_results_rwall, hold_out_dataset) {
  ...
  for(dep_var in names(hold_out_dataset)) {
    cat("\n================================================================\n")
    cat("PROCESSING", toupper(dep_var), "- ROLLING WINDOWS ALL VARIABLES\n")
    cat("================================================================\n")
    
    # Collect all SMAPE values from sensitivity analysis only
    ...
    # Use sensitivity analysis results to get SMAPE values
    if(!is.null(sensitivity_results_rwall) && dep_var %in% names(sensitivity_results_rwall)) {
      for(model_name in names(sensitivity_results_rwall[[dep_var]])) {
        model_data <- sensitivity_results_rwall[[dep_var]][[model_name]]
        ...
        for(config_name in names(model_data$results)) {
          ...
          if(!is.null(config$smape) && !is.na(config$smape)) {
            all_configs <- rbind(all_configs, 
                                 data.frame(Method = "Sensitivity_RWALL", 
                                            Window = model_data$window, 
                                            Model = model_name,
                                            Config = config_name,
                                            SMAPE = config$smape))
    ...
    if(nrow(all_configs) == 0) {
      cat("No valid configurations found for", dep_var, "\n")
      next
    }
    
    # Select top 10 configurations
    valid_configs <- all_configs[!is.na(all_configs$SMAPE), ]
    top_configs <- valid_configs[order(valid_configs$SMAPE), ][1:min(10, nrow(valid_configs)), ]
    
    cat("Top 10 configurations selected for RWALL", dep_var, ":\n")
    print(top_configs)
    ...
    for(i in 1:nrow(top_configs)) {
      ...
      tryCatch({
        # Use the corresponding model from all_models_rwall
        if(dep_var %in% names(all_models_rwall) && config$Window <= length(all_models_rwall[[dep_var]])) {
          model <- all_models_rwall[[dep_var]][[config$Window]]
          pred <- predict(model, newdata = pred_data, burn = 100)
        ...
        cat("Error predicting for config", i, ":", e$message, "\n")
    ...
    if(successful_predictions == 0) {
      cat("No successful predictions for", dep_var, "\n")
      # Still add NA results to maintain structure
      final_results <- rbind(final_results, 
                             data.frame(Variable = dep_var, Prediction_Type = "Mean", Ensemble_SMAPE = NA),
                             data.frame(Variable = dep_var, Prediction_Type = "Median", Ensemble_SMAPE = NA))
      next
    }
    
    # Remove failed predictions
    valid_cols <- !is.na(colSums(pred_matrix_mean, na.rm = TRUE))
    if(sum(valid_cols) > 0) {
...
# Run the function
ensemble_results_rwall <- combine_existing_results_rwall(all_models_rwall, sensitivity_results_rwall, hold_out_dataset)
print(ensemble_results_rwall)
...
# Excel
```{r}
# Fixed Rolling Windows All Methods - Creates RW_ALL_2015Q1.xlsx
  ...
  for(dep_var in dep_vars) {
    ...
    for(window in top_5_mean) {
      ...
      if(is.null(all_models[[dep_var]][[window]]) || !inherits(all_models[[dep_var]][[window]], "bsts")) {
        ...
        # Check if the test data exists - FIXED: use parameter name 'extended_test_sets'
        if(is.null(extended_test_sets[[dep_var]][[window]]) || 
           nrow(extended_test_sets[[dep_var]][[window]]) == 0) {
...
###############################################################################
## 3) add_sensitivity_analysis - Full version
###############################################################################
add_sensitivity_analysis <- function( sensitivity_results_rwall, file_path) {
  ...
  for(dep_var in names( sensitivity_results_rwall)) {
    ...
    # Add a small gap (6 rows) after the last content
    ...
    for (model_name in names( sensitivity_results_rwall[[dep_var]])) {
      model_info <-  sensitivity_results_rwall[[dep_var]][[model_name]]
      window <- model_info$window
...
add_influence_analysis_to_workbook <- function(influence_results, file_path) {
  ...
  for (dep_var in names(influence_results)) {
    ...
    for (model_name in names(influence_results[[dep_var]])) {
      ...
      # Map delta_test_forecast to delta_holdout_forecast if needed
...
###############################################################################
## 5) add_diagnostics_to_workbook - Fixed for rolling_splits
###############################################################################
...
###############################################################################
## 6) format_window_range - Rolling windows version
###############################################################################
format_window_range <- function(window) {
  # Define the descriptive text for each rolling window's train and test sets
  # Updated to reflect the new window specifications and hold-out period (73-79)
  train_ranges <- c(
    "1-72 & 80-87",
    "5-72 & 80-101",
    "9-72 & 80-105 ",
...
###############################################################################
## 7) create_excel_results_revised - MAIN FUNCTION - Creates RW_ALL_2015Q1.xlsx
###############################################################################
create_excel_results_revised <- function(
  ...
  file_path <- "RW_ALL_2015Q1.xlsx"  # CORRECT FILE NAME
  ...
  add_sensitivity_analysis( sensitivity_results_rwall, file_path)
...
###############################################################################
## 8) USAGE - CORRECTED VERSION - Use the main function, not the sub-function
###############################################################################
result <- create_excel_results_revised(
  all_evaluations = all_evaluations_rwall, 
  all_models = all_models_rwall, 
  extended_test_sets = extended_test_sets_rolling, 
  crps_scores = crps_scores_custom_rolling, 
  lpd_scores = lpd_scores, 
   sensitivity_results_rwall =  sensitivity_results_rwall, 
  ...
  ljung_box_results = ljung_box_results,
  aggregate_smape_results = aggregate_smape_results,
  ensemble_results = ensemble_results_rwall
)
...
# Save the workbook
saveWorkbook(wb, file = "Removed_pre_BWRW_2015.xlsx", overwrite = TRUE)
...
# Extract the hold-out period (rows 73-79) + 2024 Q4
...
analyze_bsts_mcmc_trace_plots_rolling <- function(all_evaluations, all_models, best_models) {
  # Create a PDF device to save all plots
  pdf("MCMC_trace_plot_RW_BW_2015Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets_rolling, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_RW_BW_2015Q1.pdf", width = 12, height = 8)
...
run_sensitivity_analysis_parallel <- function() {
  ...
  # Initialize results
  sensitivity_results_rwbw <- list()
  
  # For each dependent variable
  for (dep_var in names(best_models)) {
    cat("\n=== Starting analysis for", dep_var, "===\n")
    sensitivity_results_rwbw[[dep_var]] <- list()
    models_info <- best_models[[dep_var]]
    ...
    for (i in 1:min(5, nrow(models_info))) {
      ...
      # Store results for this model
      sensitivity_results_rwbw[[dep_var]][[model_name]] <- list(
        window = window_size,
        method = method,
        results = model_results
      )
  ...
  return(sensitivity_results_rwbw)
}
...
sensitivity_results_rwbw <- run_sensitivity_analysis_parallel()
...
```{r}
# Function to identify the top 5 models by SMAPE
find_top_models <- function(all_models_rwbw, sensitivity_results_rwbw) {
  top_models_summary <- list()
  
  # Process each dependent variable
  for(dep_var in names(sensitivity_results_rwbw)) {
    cat("\n================================================================\n")
    cat("TOP 5 MODELS FOR", toupper(dep_var), "BASED ON SMAPE\n")
    cat("================================================================\n")
    ...
    # Add sensitivity analysis results
    if (dep_var %in% names(sensitivity_results_rwbw)) {
      for (model_name in names(sensitivity_results_rwbw[[dep_var]])) {
        model_data <- sensitivity_results_rwbw[[dep_var]][[model_name]]
        window <- model_data$window
        method <- model_data$method
...
# Example usage:
 top_model_results <- find_top_models(all_models_rwbw, sensitivity_results_rwbw)
```
...
## ensemble result 
```{r}
combine_existing_results_rwbw <- function(all_models_rwbw, sensitivity_results_rwbw, hold_out_dataset) {
  ...
  for(dep_var in names(hold_out_dataset)) {
    cat("\n================================================================\n")
    cat("PROCESSING", toupper(dep_var), "- ROLLING WINDOWS BACKWARD ELIMINATION\n")
    cat("================================================================\n")
    ...
    # Use sensitivity analysis results to get SMAPE values
    if(!is.null(sensitivity_results_rwbw) && dep_var %in% names(sensitivity_results_rwbw)) {
      for(model_name in names(sensitivity_results_rwbw[[dep_var]])) {
        model_data <- sensitivity_results_rwbw[[dep_var]][[model_name]]
        ...
        for(config_name in names(model_data$results)) {
          ...
          if(!is.null(config$smape) && !is.na(config$smape)) {
            all_configs <- rbind(all_configs, 
                                 data.frame(Method = "Sensitivity_RWBW", 
                                            Window = model_data$window, 
                                            Model = model_name,
                                            Config = config_name,
                                            SMAPE = config$smape))
    ...
    cat("Top 10 configurations selected for RWBW", dep_var, ":\n")
    print(top_configs)
    ...
    for(i in 1:nrow(top_configs)) {
      ...
      tryCatch({
        # Use the corresponding model from all_models_rwbw
        if(dep_var %in% names(all_models_rwbw) && config$Window <= length(all_models_rwbw[[dep_var]])) {
          model <- all_models_rwbw[[dep_var]][[config$Window]]
          pred <- predict(model, newdata = pred_data, burn = 100)
          pred_matrix_mean[, i] <- colMeans(pred$distribution)
          pred_matrix_median[, i] <- apply(pred$distribution, 2, median)
          successful_predictions <- successful_predictions + 1
...
# Run the function
ensemble_results_rwbw <- combine_existing_results_rwbw(all_models_rwbw, sensitivity_results_rwbw, hold_out_dataset)
print(ensemble_results_rwbw)
```
...
```{r}
###############################################################################
## 1) create_enhanced_prediction_tables_revised - CORRECTED FUNCTION
###############################################################################
create_enhanced_prediction_tables_revised <- function(all_evaluations, all_models, extended_test_sets, 
                                              crps_scores, lpd_scores, file_path, 
                                              aggregate_smape_results,
  ...
  for(dep_var in dep_vars) {
    ...
    for(window in top_5_mean) {
      ...
      if(is.null(all_models[[dep_var]][[window]]) || !inherits(all_models[[dep_var]][[window]], "bsts")) {
        ...
        # Check if the test data exists - FIXED: use correct variable name
        if(is.null(extended_test_sets[[dep_var]][[window]]) || 
           nrow(extended_test_sets[[dep_var]][[window]]) == 0) {
          # Test data not available
          writeData(wb, dep_var, "Test data not available", startRow = current_row, startCol = 2)
...
###############################################################################
## 2) add_sensitivity_analysis - Full version
###############################################################################
add_sensitivity_analysis <- function(sensitivity_results_rwbw, file_path) {
  wb <- loadWorkbook(file_path)
  ...
  for(dep_var in names(sensitivity_results_rwbw)) {
    ...
    for (model_name in names(sensitivity_results_rwbw[[dep_var]])) {
      model_info <- sensitivity_results_rwbw[[dep_var]][[model_name]]
      window <- model_info$window
      method <- model_info$method
...
###############################################################################
## 3) add_influence_analysis_to_workbook - Complete influence analysis
###############################################################################
add_influence_analysis_to_workbook <- function(influence_results, file_path) {
  wb <- loadWorkbook(file_path)
...
###############################################################################
## 4) add_diagnostics_to_workbook - Fixed for rolling_splits
###############################################################################
add_diagnostics_to_workbook <- function(all_models, rolling_splits, all_diagnostics, all_looic, 
                                        bayesian_R2_results, crps_scores, lpd_scores, 
                                        ljung_box_results, file_path) {
...
###############################################################################
## 5) format_window_range - Rolling windows version
###############################################################################
format_window_range <- function(window) {
  train_ranges <- c(
    "1-72 & 80-87",
...
###############################################################################
## 6) create_excel_results_revised - MAIN FUNCTION - Creates RW_BW_2015Q1.xlsx
###############################################################################
create_excel_results_revised <- function(
  all_evaluations, 
  all_models, 
  ...
  file_path <- "RW_BW_2015Q1.xlsx"  # CORRECT FILE NAME
  ...
  cat("Adding sensitivity analysis...\n")
  add_sensitivity_analysis(sensitivity_results_rwbw, file_path)
...
###############################################################################
## 7) CORRECTED USAGE - Use the main wrapper function
###############################################################################

# CORRECT WAY: Use the main wrapper function that handles everything
result <- create_excel_results_revised(
  ...
  sensitivity_results_rwbw = sensitivity_results_rwbw, 
  influence_results = influence_results_parallel, 
  rolling_splits = rolling_splits,  
  ...
  aggregate_smape_results = aggregate_smape_results,
  ensemble_results = ensemble_results_rwbw 
)
...
run_best_selection <- function(best_models, rolling_splits, min_vars = 15) {
  all_models_ewstep <- list()
  
  for (dep_var in names(best_models)) {
    cat("\n\n==== Variable Selection for", dep_var, "====\n")
    
    # Skip if this variable doesn't exist in rolling_splits
    if (!dep_var %in% names(rolling_splits)) {
    ...
    if (best_window < 1 || best_window > length(rolling_splits[[dep_var]])) {
      cat("Window index", best_window, "out of range for", dep_var, "\n")
      cat("Available indices: 1 to", length(rolling_splits[[dep_var]]), "\n")
    ...
    train_data <- rolling_splits[[dep_var]][[best_window]]$train
...
# Save the workbook
saveWorkbook(wb, file = "Removed_pre_StepRW_2015.xlsx", overwrite = TRUE)
...
analyze_bsts_mcmc_trace_plots_rolling <- function(all_evaluations, all_models_rwstep, best_models) {
  # Create a PDF device to save all plots
  pdf("MCMC_trace_plot_RW_step_2015Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets_rolling, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_RW_Step_2015Q1.pdf", width = 12, height = 8)
...
run_sensitivity_analysis_parallel <- function() {
  ...
  # Initialize results
  sensitivity_results_rwstep <- list()
  
  # For each dependent variable
  for (dep_var in names(best_models)) {
    cat("\n=== Starting analysis for", dep_var, "===\n")
    sensitivity_results_rwstep[[dep_var]] <- list()
    models_info <- best_models[[dep_var]]
    ...
    for (i in 1:min(5, nrow(models_info))) {
      ...
      # Store results for this model
      sensitivity_results_rwstep[[dep_var]][[model_name]] <- list(
        window = window_size,
        method = method,
        results = model_results
      )
  ...
  return(sensitivity_results_rwstep)
}
...
sensitivity_results_rwstep <- run_sensitivity_analysis_parallel()
...
```{r}
# Function to identify the top 5 models by SMAPE
find_top_models <- function(all_models_rwstep, sensitivity_results_rwstep) {
  top_models_summary <- list()
  
  # Process each dependent variable
  for(dep_var in names(sensitivity_results_rwstep)) {
    cat("\n================================================================\n")
    cat("TOP 5 MODELS FOR", toupper(dep_var), "BASED ON SMAPE\n")
    cat("================================================================\n")
    ...
    # Add sensitivity analysis results
    if (dep_var %in% names(sensitivity_results_rwstep)) {
      for (model_name in names(sensitivity_results_rwstep[[dep_var]])) {
        model_data <- sensitivity_results_rwstep[[dep_var]][[model_name]]
        window <- model_data$window
        method <- model_data$method
...
# Example usage:
 top_model_results <- find_top_models(all_models_rwstep, sensitivity_results_rwstep)
```
...
## ensemble result 
```{r}
combine_existing_results_rwstep <- function(all_models_rwstep, sensitivity_results_rwstep, hold_out_dataset) {
  ...
  for(dep_var in names(hold_out_dataset)) {
    cat("\n================================================================\n")
    cat("PROCESSING", toupper(dep_var), "- ROLLING WINDOWS STEPWISE\n")
    cat("================================================================\n")
    ...
    # Use sensitivity analysis results to get SMAPE values
    if(!is.null(sensitivity_results_rwstep) && dep_var %in% names(sensitivity_results_rwstep)) {
      for(model_name in names(sensitivity_results_rwstep[[dep_var]])) {
        model_data <- sensitivity_results_rwstep[[dep_var]][[model_name]]
        ...
        for(config_name in names(model_data$results)) {
          ...
          if(!is.null(config$smape) && !is.na(config$smape)) {
            all_configs <- rbind(all_configs, 
                                 data.frame(Method = "Sensitivity_RWSTEP", 
                                            Window = model_data$window, 
                                            Model = model_name,
                                            Config = config_name,
                                            SMAPE = config$smape))
    ...
    cat("Top 10 configurations selected for RWSTEP", dep_var, ":\n")
    print(top_configs)
    ...
    for(i in 1:nrow(top_configs)) {
      ...
      tryCatch({
        # Use the corresponding model from all_models_rwstep
        if(dep_var %in% names(all_models_rwstep) && config$Window <= length(all_models_rwstep[[dep_var]])) {
          model <- all_models_rwstep[[dep_var]][[config$Window]]
          pred <- predict(model, newdata = pred_data, burn = 100)
          pred_matrix_mean[, i] <- colMeans(pred$distribution)
          pred_matrix_median[, i] <- apply(pred$distribution, 2, median)
          successful_predictions <- successful_predictions + 1
...
# Run the function
ensemble_results_rwstep <- combine_existing_results_rwstep(all_models_rwstep, sensitivity_results_rwstep, hold_out_dataset)
print(ensemble_results_rwstep)
```
...
# Excel
```{r}
# Complete Corrected Excel Creation Code for Rolling Windows with Stepwise Elimination
# This creates RW_Step_2015Q1.xlsx

###############################################################################
## 1) create_enhanced_prediction_tables_revised - CORRECTED FUNCTION FOR STEPWISE
###############################################################################
create_enhanced_prediction_tables_revised <- function(all_evaluations, all_models, extended_test_sets, 
                                              crps_scores, lpd_scores, file_path, 
                                              aggregate_smape_results,
...
###############################################################################
## 6) create_excel_results_revised - MAIN FUNCTION - Creates RW_Step_2015Q1.xlsx
###############################################################################
create_excel_results_revised <- function(
  all_evaluations, 
  all_models, 
  ...
  file_path <- "RW_Step_2015Q1.xlsx"  # CORRECT FILE NAME FOR STEPWISE
...
###############################################################################
## 7) CORRECTED USAGE - Use the main wrapper function for STEPWISE
###############################################################################


# CORRECT WAY: Use the main wrapper function that handles everything for STEPWISE
result <- create_excel_results_revised(
  ...
  sensitivity_results = sensitivity_results_rwstep, 
  influence_results = influence_results_parallel, 
  rolling_splits = rolling_splits,  
  ...
  aggregate_smape_results = aggregate_smape_results,
  ensemble_results = ensemble_results_rwstep
)
