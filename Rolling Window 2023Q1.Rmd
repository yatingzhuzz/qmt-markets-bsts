...
predict_all_rolling_windows_holdout <- function(all_models_rolling, hold_out_dataset) {
  ...
  for(window_num in 1:5) {  # 5 rolling windows
    ...
    for(var_name in names(all_models_rolling)) {
      ...
      if(num_rows <= 7) {
        row_ids <- 105:111#period
      } else {
        row_ids <- c(105:111, 112)#period
      }
...
analyze_bsts_mcmc_trace_plots_rolling <- function(all_evaluations, all_models, best_models) {
  ...
  pdf("MCMC_trace_plot_RW_ALL_2023Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets_rolling, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_RW_ALL_2023Q1.pdf", width = 12, height = 8)
...
format_window_range <- function(window) {
  ...
  # Updated to reflect the new window specifications and hold-out period (27-33)
  train_ranges <- c(
    "1 - 90 ",
    "5 - 94",
    "9 - 98",
...
create_excel_results_revised <- function(
  ...
  file_path <- "RW_ALL_2023Q1.xlsx"  # CORRECT FILE NAME
...
# Save the workbook
saveWorkbook(wb, file = "Removed_pre_BWRW_2023.xlsx", overwrite = TRUE)
...
# Extract the hold-out period (rows 80-86) + 2024 Q1
...
analyze_bsts_mcmc_trace_plots_rolling <- function(all_evaluations, all_models, best_models) {
  # Create a PDF device to save all plots
  pdf("MCMC_trace_plot_RW_BW_2023Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets_rolling, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_RW_BW_2023Q1.pdf", width = 12, height = 8)
...
create_excel_results_revised <- function(
  ...
  file_path <- "RW_BW_2023Q1.xlsx"  # CORRECT FILE NAME
...
# Save the workbook
saveWorkbook(wb, file = "Removed_pre_StepRW_2023.xlsx", overwrite = TRUE)
...
analyze_bsts_mcmc_trace_plots_rolling <- function(all_evaluations, all_models_rwstep, best_models) {
  # Create a PDF device to save all plots
  pdf("MCMC_trace_plot_RW_step_2023Q1.pdf", width=12, height=10)
...
create_prediction_plots <- function(all_evaluations, all_models, extended_test_sets_rolling, hold_out_dataset) {
  library(ggplot2)
  pdf("Prediction_Distribution_RW_Step_2023Q1.pdf", width = 12, height = 8)
...
## 2) add_sensitivity_analysis - Full version
###############################################################################
add_sensitivity_analysis <- function(sensitivity_results_rwstep, file_path) {
  wb <- loadWorkbook(file_path)
  ...
  for(dep_var in names(sensitivity_results_rwstep)) {
    ...
    for (model_name in names(sensitivity_results_rwstep[[dep_var]])) {
      model_info <- sensitivity_results_rwstep[[dep_var]][[model_name]]
      window <- model_info$window
      method <- model_info$method
...
create_excel_results_revised <- function(
  ...
  file_path <- "RW_Step_2023Q1.xlsx"  # CORRECT FILE NAME FOR STEPWISE
  ...
  cat("Adding sensitivity analysis...\n")
  add_sensitivity_analysis(sensitivity_results_rwstep, file_path)
...
result <- create_excel_results_revised(
  ...
  sensitivity_results_rwstep = sensitivity_results_rwstep, 
  influence_results = influence_results_parallel, 
  rolling_splits = rolling_splits,  
